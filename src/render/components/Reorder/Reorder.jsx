...
class Reorder extends Component {
  ...
  validate() {
    const { selectedParts, original, directory } = this.state;
    const errors = {};
    if (this.state.new.length < 6) errors.new = 'Invalid Number';
    if (original.length < 9) errors.original = 'Invalid Number';
    if (selectedParts.length < 1) errors.selectedParts = 'No Parts Selected';
    if (directory.length < 1) errors.directory = 'No Directory Selected';
      ...
      <ListItem className={classes.root}>
        ...
        <form style={{ marginLeft: '20px' }}>
          <FormControl className={classes.formControl}>
            <InputLabel htmlFor="select-multiple">Directory</InputLabel>
            <Select
